...
\begin{center}
\begin{tikzpicture}[
  block/.style={rectangle, draw, minimum width=1.6cm, minimum height=0.85cm, font=\sffamily\small, align=center},
  node distance=1.7cm,
  font=\small,
  scale=0.88,
...
\node[block, right of=data, node distance=2cm] (mapper) {Symbol\\Mapper};
\node[right of=mapper, node distance=1.8cm] (split) {};

% I path
\node[block, above right of=split, node distance=2cm] (dac_i) {DAC};
\node[block, right of=dac_i, node distance=2cm] (mult_i) {Mixer\\$\times$};

% Q path  
\node[block, below right of=split, node distance=2cm] (dac_q) {DAC};
\node[block, right of=dac_q, node distance=2cm] (mult_q) {Mixer\\$\times$};

% Local oscillator
\node[block, below of=mult_q, node distance=1.7cm] (lo) {Local\\Oscillator};
\node[block, right of=lo, node distance=2cm] (phase) {$90^\circ$\\Shifter};

% Combiner
\node[block, right of=mult_i, node distance=2cm, yshift=-1.25cm] (sum) {Combiner\\$\Sigma$};
\node[block, right of=sum, node distance=2cm] (amp) {PA};
\node[align=center, right of=amp, node distance=1.8cm] (output) {\sffamily RF\\Output};
...
\begin{tikzpicture}[
  ...
  font=\small,
  scale=0.83,
  every node/.style={scale=0.83}
]
% Input
\node[align=center] (input) {\sffamily RF\\Input};
\node[block, right of=input, node distance=1.8cm] (lna) {LNA};
\node[right of=lna, node distance=1.5cm] (split) {};

% I path
\node[block, above right of=split, node distance=2cm] (mult_i) {Mixer\\$\times$};
\node[block, right of=mult_i, node distance=2cm] (lpf_i) {Lowpass\\Filter};
\node[block, right of=lpf_i, node distance=2cm] (adc_i) {ADC};

% Q path
\node[block, below right of=split, node distance=2cm] (mult_q) {Mixer\\$\times$};
\node[block, right of=mult_q, node distance=2cm] (lpf_q) {Lowpass\\Filter};
\node[block, right of=lpf_q, node distance=2cm] (adc_q) {ADC};
...
% Output
\node[right of=adc_i, node distance=1.8cm, yshift=-1.25cm] (combine) {};
\node[block, right of=combine, node distance=1.7cm] (dsp) {DSP};
\node[align=center, right of=dsp, node distance=1.8cm] (output) {\sffamily Data\\Output};
