...
\begin{tikzpicture}[
  scale=0.6,
  block/.style={rectangle, draw, minimum width=1.5cm, minimum height=0.7cm, font=\sffamily\tiny, align=center},
...
% Input
\node[font=\tiny] (input) {\textbf{Pulsed}\\\textbf{Microwaves}\\1--10 GHz};

% First row of process blocks
\node[block, right=of input] (heating) {Rapid\\Tissue\\Heating};
\node[block, right=of heating] (expansion) {Thermoelastic\\Expansion};

% Second row of process blocks
\node[block, below=1.0cm of heating, xshift=1.2cm] (acoustic) {Pressure\\Wave};
\node[block, right=of acoustic] (cochlea) {Cochlear\\Stimulation};

% Output
\node[right=of cochlea, font=\tiny] (output) {\textbf{Auditory}\\\textbf{Perception}\\200 Hz--10 kHz};

% Arrows with labels
\draw[arrow] (input) -- (heating);
\draw[arrow] (heating) -- node[above, font=\tiny] {$\Delta T \sim$ µK} (expansion);
\draw[arrow] (expansion) -- (expansion |- acoustic.north) -- node[left, font=\tiny] {$p \sim$ Pa} (acoustic);
\draw[arrow] (acoustic) -- (cochlea);
\draw[arrow] (cochlea) -- (output);

% Time scale annotations
\node[above=0.2cm of heating, font=\tiny, align=center] {$\tau_p \sim$ 1--50 µs\\(pulse duration)};
\node[above=0.2cm of expansion, font=\tiny, align=center] {$\tau_{th} \sim$ 1 µs\\(thermal time)};
\node[below=0.2cm of acoustic, font=\tiny, align=center] {$f_{PRF}$\\(repetition rate)};

% Title
\node[above=0.8cm of heating, font=\tiny\bfseries, xshift=1.2cm] {Frey Microwave Auditory Effect: Energy Transduction Pathway};

% EM to Acoustic conversion box
\draw[thick, dashed, NavyBlue] ([xshift=-0.2cm, yshift=0.3cm]heating.north west) rectangle ([xshift=0.2cm, yshift=-0.2cm]acoustic.south east);
\node[left, NavyBlue, font=\tiny, align=right] at ([xshift=-0.3cm]heating.west |- acoustic) {EM $\rightarrow$\\Acoustic\\Transduction};
\end{tikzpicture}
\end{center}
