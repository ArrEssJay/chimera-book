...
\draw[->,thick] (-0.5,0) -- (10.5,0) node[right] {\sffamily\small Code Space};

% Codeword 1
\fill[black] (1,0) circle (4pt);
\node[below=8pt] at (1,0) {\sffamily\small $c_1$};
\draw[dashed,gray!50] (1,0) circle (1.3cm);
\node[above=6pt,gray,font=\scriptsize] at (1,1.3) {Error sphere};

% Codeword 2
\fill[black] (5,0) circle (4pt);
\node[below=8pt] at (5,0) {\sffamily\small $c_2$};
\draw[dashed,gray!50] (5,0) circle (1.3cm);

% Codeword 3
\fill[black] (9,0) circle (4pt);
\node[below=8pt] at (9,0) {\sffamily\small $c_3$};
\draw[dashed,gray!50] (9,0) circle (1.3cm);

% Distance annotation
\draw[<->,thick] (1,2.3) -- (5,2.3) node[midway,above] {\sffamily\small $d_{\min} = 4$};

% Received word example
\fill[red] (2.2,0) circle (3pt);
\node[below=16pt,red,font=\scriptsize,align=center] at (2.2,0) {Received\\word};
\draw[->,red,thick] (2.2,0.1) -- (1.1,0.1);
\node[above=10pt,red,font=\scriptsize] at (1.65,0.1) {Decode to $c_1$};

% Error detection region
\draw[thick,red!70!black,dashed] (1,0.5) -- (5,0.5);
\node[above,red!70!black,font=\scriptsize] at (3,0.5) {Detection region};
\end{tikzpicture}
\end{center}
...
\node[font=\sffamily\bfseries] at (9.5,3.5) {Error Correction ($d_{\min} = 3$)};
