...
% Time axis
\draw[->] (0,0) -- (11,0) node[right,font=\sffamily\small] {Time};
...
% Legend
\node[font=\sffamily\footnotesize,align=left] at (8.5,1.2) {
  Each symbol carries\\
  multiple bits\\[2pt]
  $E_s = k \cdot E_b$\\
  where $k$ = bits/symbol
...
\begin{tikzpicture}[scale=1.0]
% Frequency axis
\draw[->] (0,0) -- (10,0) node[right,font=\sffamily\small] {Frequency (Hz)};
\draw[->] (0,0) -- (0,3.5) node[above,font=\sffamily\small] {Power};

% Noise floor
\fill[gray!20] (0,0) rectangle (9.5,1.5);
\draw[thick,gray] (0,1.5) -- (9.5,1.5);
\node[right,font=\sffamily\small] at (9.5,1.5) {$N_0$ (W/Hz)};
...
% Annotations
\draw[<->,thick] (8.2,0) -- (8.2,1.5) node[midway,right,font=\scriptsize,align=left] {Noise\\power\\per Hz};
...
\begin{tikzpicture}[scale=1.0]
% Axes
\draw[->] (0,0) -- (0,5.5) node[above,font=\sffamily\small] {BER (log scale)};
\draw[->] (0,0) -- (10,0) node[right,font=\sffamily\small] {$E_b/N_0$ (dB)};
...
\node[left,font=\scriptsize] at (0,4) {$10^{-3}$};
\node[left,font=\scriptsize] at (0,3) {$10^{-5}$};
\node[left,font=\scriptsize] at (0,2) {$10^{-7}$};
\node[left,font=\scriptsize] at (0,1) {$10^{-9}$};
...
\node[green!60!black,font=\sffamily\small] at (9.5,3.5) {16-QAM};

% Shannon limit line
\draw[thick,dashed,black] (-0.5,0) -- (-0.5,5.5);
\node[font=\scriptsize,align=center] at (-0.5,5.8) {Shannon\\Limit\\$-1.59$~dB};

% Operating points
\fill[black] (10,2.5) circle (2pt);
\node[font=\scriptsize,align=left] at (10,2.9) {Typical\\operating\\point};
