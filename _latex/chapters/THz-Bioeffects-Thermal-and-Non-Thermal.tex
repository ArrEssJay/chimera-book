...
\fill[red] (0.5,{2/(0.5*0.5)}) circle (1.5pt) node[above right,font=\tiny] {0.5 THz};
\fill[red] (1,2) circle (1.5pt) node[right,font=\tiny] {1 THz: 200 µm};
\fill[red] (2,0.5) circle (1.5pt) node[above right,font=\tiny] {2 THz};
\fill[red] (3,{2/9}) circle (1.5pt);
\fill[red] (4,{2/16}) circle (1.5pt);

% Annotation
\node[align=center,font=\sffamily\footnotesize] at (3.5,3.2) {Penetration depth\\$\delta = 1/\alpha(f)$};
...
\draw[->] (0,0) -- (5.5,0) node[right] {\sffamily\small Depth $z$ (µm)};
\draw[->] (0,0) -- (0,4) node[above] {\sffamily\small Temperature};

% Grid
\draw[very thin,gray!30] (0,0) grid[step=0.5] (5.5,4);
...
\draw[thick,blue] plot[domain=0:5,samples=50] (\x,{3.6*exp(-\x/1)});
\draw[thick,red] plot[domain=0:5,samples=50] (\x,{2.4*exp(-\x/1.5)});
\draw[thick,green!50!black] plot[domain=0:5,samples=50] (\x,{1.2*exp(-\x/2)});

% Legend positioned more compactly
\node[font=\tiny,blue] at (5.2,3.4) {$t=0$};
\node[font=\tiny,red] at (5.2,2.4) {$t=50$ ms};
\node[font=\tiny,green!50!black] at (5.2,1.4) {$t=100$ ms};

% Surface heating zone
\fill[yellow!30,opacity=0.3] (0,0) rectangle (1,3.6);
\node[font=\tiny,align=center] at (0.5,3.9) {Surface\\heating};
...
% Annotation
\node[align=center,font=\sffamily\footnotesize] at (3.5,3.5) {Heat diffuses\\into tissue};
\draw[->,thick] (3.5,3.2) -- (2,1.5);
\end{tikzpicture}
\end{center}
...
\fill[green!20] (0,0) rectangle (5.5,1);
\fill[yellow!30] (0,1) rectangle (5.5,2);
\fill[orange!30] (0,2) rectangle (5.5,5);
\fill[red!30] (0,5) rectangle (5.5,8);

% Zone labels
\node[right,align=left,font=\footnotesize] at (0.2,0.5) {\textbf{Safe:} No effects\\Thermal fluctuations};
\node[right,align=left,font=\footnotesize] at (0.2,1.5) {\textbf{Mild:} Altered metabolism\\Enhanced blood flow};
\node[right,align=left,font=\footnotesize] at (0.2,3.5) {\textbf{Moderate:} Protein denaturation\\Enzyme inactivation\\Cell membrane disruption};
\node[right,align=left,font=\footnotesize] at (0.2,6.5) {\textbf{Severe:} Tissue ablation\\Burns\\Apoptosis};
...
% ICNIRP limit
\draw[blue,very thick,dashed] (0,1) -- (5.5,1);
\node[right,blue,font=\footnotesize\bfseries] at (5.6,1) {ICNIRP Limit};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[
  block/.style={rectangle, draw, minimum width=2.5cm, minimum height=1cm, font=\sffamily\footnotesize, align=center},
  node distance=2cm,
  font=\footnotesize,
  scale=0.95,
...
% Two pathways
\node[block,fill=red!20,below left=1.3cm and 1.2cm of thz] (thermal) {Thermal\\Mechanism};
\node[block,fill=blue!20,below right=1.3cm and 1.2cm of thz] (nonthermal) {Non-Thermal?\\Mechanism};

% Thermal pathway details
\node[block,below=1cm of thermal,font=\tiny] (heat) {Absorption $\rightarrow$\\Heat Generation\\$Q = \alpha I$};
\node[block,below=1cm of heat,font=\tiny] (temprise) {Temperature\\Rise $\Delta T$};
\node[block,below=1cm of temprise,font=\tiny,fill=green!20] (damage) {Cell Damage\\if $\Delta T > 1°$C};

% Non-thermal pathway details
\node[block,below=1cm of nonthermal,font=\tiny] (resonance) {Resonant\\Excitation?};
\node[block,below=1cm of resonance,font=\tiny] (signal) {Signaling\\Cascade?};
\node[block,below=1cm of signal,font=\tiny,fill=orange!20] (effect) {Biological\\Effect?};
...
% Labels
\node[font=\tiny\bfseries,red] at (-2.2,-3.8) {ESTABLISHED};
\node[font=\tiny\bfseries,blue] at (2.2,-3.8) {CONTROVERSIAL};

% Question marks
\node[font=\large,blue] at (2.6,-2.2) {?};
\node[font=\large,blue] at (2.6,-3.8) {?};
\node[font=\large,blue] at (2.6,-5.4) {?};
...
\begin{tikzpicture}[scale=1.2]
% Axes
\draw[->] (0,0) -- (6,0) node[right] {\sffamily\footnotesize Power Density (mW/cm$^2$)};
\draw[->] (0,0) -- (0,4.5) node[above] {\sffamily\footnotesize Exposure Category};

% Bars for different exposure categories
% Safe zone (0-2)
\fill[green!40] (0,0.5) rectangle (2,1.1);
\node[right,font=\footnotesize] at (2.05,0.8) {ICNIRP Public: 2 mW/cm$^2$};

% Occupational (0-10)
\fill[yellow!40] (0,1.6) rectangle (10/1.67,2.2);
\node[right,font=\footnotesize] at (6.05,1.9) {ICNIRP Occupational: 10 mW/cm$^2$};

% Typical applications
\fill[blue!30] (0,2.7) rectangle (0.5/1.67,3.3);
\node[right,font=\footnotesize] at (0.35,3.0) {Security Scanner: 0.5 mW/cm$^2$};

\fill[blue!30] (0,3.5) rectangle (1/1.67,4.1);
\node[right,font=\footnotesize] at (0.65,3.8) {Medical Imaging: 1 mW/cm$^2$};

% Grid lines
\draw[very thin,gray!30] (0,0) grid[xstep=1,ystep=0.5] (6,4.5);

% X-axis ticks
\foreach \x in {0,2,4,6,8,10}
  \draw ({\x/1.67},0) -- ({\x/1.67},-0.1) node[below,font=\tiny] {\x};

% Danger zone marker
\draw[red,very thick,dashed] (10/1.67,0) -- (10/1.67,4.5);
\node[red,font=\footnotesize\bfseries,align=center] at (5,4.35) {Exceeds\\Limit};

% Safe margin annotation
\draw[<->,thick,green!50!black] (2/1.67,0.25) -- (10/1.67,0.25);
\node[below,font=\tiny,green!50!black] at (3.6,0.25) {5× safety margin};
