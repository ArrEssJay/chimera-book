...
\textbf{Typical values:} $\pm 0.5$~dB gain, $\pm 2^\circ$ phase
...
\node[below=12pt,font=\small] at (0,-2) {16 points, 4 bits/symbol};
\node[below=24pt,font=\scriptsize,align=center] at (0,-2) {Multiple amplitudes};
\end{scope}
\end{tikzpicture}
\end{center}
...
\item \textbf{Chapter~\ref{ch:constellation-diagrams}:} Constellation Diagrams---visualization and interpretation of QAM signals
\item \textbf{Chapter~\ref{ch:iq-representation}:} IQ Representation---mathematical foundation of quadrature modulation
...
\item \textbf{Chapter~\ref{ch:amc}:} Adaptive Modulation \& Coding---dynamic QAM order selection
...
\item \textbf{Chapter~\ref{ch:channel-models-rayleigh-rician}:} Channel Models---understand fading effects on QAM performance
\item \textbf{Chapter~\ref{ch:ask}:} Amplitude Shift Keying---amplitude-only modulation component
\item \textbf{Chapter~\ref{ch:bpsk}:} Binary Phase Shift Keying---phase-only modulation component
\item \textbf{Chapter~\ref{ch:shannon-capacity}:} Shannon's Channel Capacity---theoretical limits of spectral efficiency
\end{itemize}
