...
% Bottom annotation (moved up to avoid overlap)
\node[below, font=\scriptsize, align=center] at (5, -2.5) {$E/H = 377~\Omega$ (far field)\\Perpendicular \& in phase};
\end{tikzpicture}
\end{center}
...
% Grid for reference (drawn first to avoid overlapping text)
...
\node[red, font=\scriptsize] at (2.5, -2.8) {$E/H = 377~\Omega$};
...
\begin{center}
\begin{tikzpicture}[scale=1.0]
% Grid reference (drawn first to avoid overlapping text)
...
\node[blue, font=\scriptsize, align=center, fill=white, inner sep=1pt] at (1.06, 1.06) {$S_1 = \frac{P_t}{4\pi r_1^2}$};

\draw[thick, red] (0,0) circle (3.0);
\node[red, font=\small] at (3.0, 3.0) {$r_2 = 2r_1$};
\node[red, font=\scriptsize, align=center, fill=white, inner sep=1pt] at (2.12, 2.12) {$S_2 = \frac{S_1}{4}$};

% Radial lines
\draw[->, thick] (0,0) -- (45:1.5);
...
% Bottom annotation (with better spacing)
\node[below, font=\small, align=center] at (0, -4.0) {Inverse Square Law: $S \propto \frac{1}{r^2}$};
\end{tikzpicture}
\end{center}
