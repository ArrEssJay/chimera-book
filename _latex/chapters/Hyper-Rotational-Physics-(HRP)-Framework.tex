...
\node[below,align=center,font=\small] at (0,-2.2) {Microtubule Network\\$\sim10^{14}$ tubulins};

% Wave interference pattern
\draw[thick,red,->] (-3,0) -- (-3.8,0.8) node[left,font=\scriptsize] {THz};
\draw[thick,red,->] (-3,0.5) -- (-3.9,0.7);
\draw[thick,red,->] (-3,-0.5) -- (-3.9,-0.7);
\draw[thick,red,->] (-3,0) -- (-3.8,-0.8) node[left,font=\scriptsize] {0.5--5 THz};

% Coupling arrow
\draw[->,ultra thick,orange] (3.2,0) -- (4.5,0) node[right,align=center,font=\small] {Couples to\\Bulk Geometry};
\end{tikzpicture}
\end{center}
...
\noindent where $\lambda_s$ is the soliton coherence length.
...
\draw[thick,fill=blue!10] (-3.5,-0.3) -- (3.5,-0.3) -- (3.5,0.3) -- (-3.5,0.3) -- cycle;
\node at (0,0) {\sffamily\bfseries Home Brane (Our Reality)};

% Adjacent brane
\draw[thick,fill=red!10] (-2.5,2.2) -- (2.5,2.8) -- (2.5,3.2) -- (-2.5,2.6) -- cycle;
\node at (0,2.7) {\sffamily\small Adjacent Brane};

% Bulk space
\node[gray,font=\small] at (-4.2,1.5) {Bulk Space (11D)};
\draw[->,gray,thick] (-3.7,1.5) -- (-2.9,1.5);

% Rotation indication
\draw[->,ultra thick,orange] (0,-0.3) arc[start angle=270,end angle=290,radius=2.5];
\node[orange,right,font=\small] at (0.8,-1.3) {Rotation from\\CHIMERA field};

% Operator representation
\draw[thick,fill=green!20] (-0.5,-0.3) rectangle (0.5,0.3);
\node[font=\scriptsize] at (0,-0.7) {Operator};

% Torque arrow
\draw[->,very thick,purple] (0,0) -- (1.3,1.8) node[right,font=\scriptsize,align=left] {Hyper-\\dimensional\\Torque};

% Distance annotation
\draw[<->,dashed] (3.5,0.3) -- (2.5,2.6) node[midway,right,font=\scriptsize] {$d_{\text{brane}}$};
\end{tikzpicture}
\end{center}
...
% Adjacent branes with more spacing
\node[draw,rectangle,minimum width=2cm,minimum height=1cm,fill=red!15] (prima) at (-3,2.3) {\sffamily\small BRANE-PRIMA};
\node[draw,rectangle,minimum width=2cm,minimum height=1cm,fill=orange!15] (typhon) at (3,2.3) {\sffamily\small BRANE-TYPHON};
\node[draw,rectangle,minimum width=2cm,minimum height=1cm,fill=purple!15] (ortho) at (3,-2.3) {\sffamily\small BRANE-ORTHO};
\node[draw,rectangle,minimum width=2cm,minimum height=1cm,fill=green!15] (aether) at (-3,-2.3) {\sffamily\small BRANE-AETHER};
...
% Risk indicators with better spacing
\node[below=2pt,font=\tiny,red] at (prima.south) {Risk: Low};
\node[below=2pt,font=\tiny,red] at (typhon.south) {Risk: High};
\node[below=2pt,font=\tiny,red] at (ortho.south) {Risk: Extreme};
\node[below=2pt,font=\tiny,green!50!black] at (aether.south) {Risk: Low};
\end{tikzpicture}
\end{center}
