...
% Distance annotation - positioned below to avoid overlap with labels
\draw[<->,thick] (1.06,-1.5) -- (-1.06,-1.5) node[midway,below,font=\scriptsize] {$d = \sqrt{2}A$};
\draw[<->,thick,dashed] (1.06,1.06) -- (-1.06,-1.06) node[midway,below right,font=\scriptsize] {$d_{diag} = 2A$};
\end{tikzpicture}
\end{center}
...
\node[block, above right of=input, node distance=2.3cm, yshift=0.8cm] (multi) {Mixer\\$\times$};
\node[block, below right of=input, node distance=2.3cm, yshift=-0.8cm] (multq) {Mixer\\$\times$};
\node[block, right of=multi, node distance=2.3cm] (lpfi) {Lowpass\\Filter};
\node[block, right of=multq, node distance=2.3cm] (lpfq) {Lowpass\\Filter};
\node[block, right of=lpfi, node distance=2.3cm] (sampi) {Sample\\$@T_s$};
\node[block, right of=lpfq, node distance=2.3cm] (sampq) {Sample\\$@T_s$};
\node[block, right of=sampi, node distance=2.3cm] (threshi) {Threshold\\$>0?$};
\node[block, right of=sampq, node distance=2.3cm] (threshq) {Threshold\\$>0?$};
\node[block, right of=threshi, node distance=2.3cm, yshift=-1.6cm] (mux) {Parallel to\\Serial};
\node[right of=mux, node distance=2.5cm] (output) {\sffamily Data\\$\{0,1\}$};
