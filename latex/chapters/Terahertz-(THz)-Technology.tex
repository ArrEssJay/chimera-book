...
\draw[->,thick,red,decorate,decoration={snake,amplitude=1pt}] (0.45,2) -- (3,2) node[right,font=\scriptsize] {THz photon};
\draw[->,thick,red,decorate,decoration={snake,amplitude=1pt}] (0.75,1) -- (3,1) node[right,font=\scriptsize] {THz photon};
...
% Free space
\node[right of=txant, node distance=2.5cm] (space) {\sffamily Target};

% Receiver chain
\node[block, right of=space, node distance=2.5cm] (rxant) {RX\\Antenna};
\node[block, right of=rxant] (mixer) {Detector\\(Bolometer)};
\node[block, right of=mixer] (proc) {Signal\\Processing};

% Arrows
\draw[->,thick] (qcl) -- node[above] {\scriptsize CW THz} (modulator);
\draw[->,thick] (modulator) -- node[above] {\scriptsize modulated} (txant);
\draw[->,thick,red,decorate,decoration={snake,amplitude=1pt}] (txant) -- (space);
\draw[->,thick,red,decorate,decoration={snake,amplitude=1pt}] (space) -- node[above] {\scriptsize reflected} (rxant);
\draw[->,thick] (rxant) -- (mixer);
\draw[->,thick] (mixer) -- node[above] {\scriptsize baseband} (proc);
...
% Lock-in reference
\draw[->,thick,dashed] (drive.east) -| (proc.south) node[pos=0.25,right,font=\scriptsize] {ref};

% Labels
\node[above,font=\sffamily\small] at (modulator.north) [yshift=0.8cm] {Transmitter};
\node[above,font=\sffamily\small] at (mixer.north) [yshift=0.8cm] {Receiver};
\end{tikzpicture}
\end{center}
