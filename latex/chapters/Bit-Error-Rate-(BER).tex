...
\node (tx) [block, right of=prbs, node distance=2.8cm] {Transmitter\\(Modulator)};
\node (channel) [block, right of=tx, node distance=3cm] {Channel\\Under Test};

% Receiver side
\node (rx) [block, right of=channel, node distance=3cm] {Receiver\\(Demodulator)};
\node (comp) [block, right of=rx, node distance=2.8cm] {Error\\Comparator};
