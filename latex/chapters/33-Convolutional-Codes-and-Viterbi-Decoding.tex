...
% TODO: Fix TikZ trellis diagram - currently causes TeX capacity exceeded error
% The diagram below has been temporarily commented out to allow the book to build.
% Issue: Complex nested \foreach loops with TikZ node naming causing infinite recursion
...
%   \draw[solid] (s0-t\t) -- (s0-t\tnext) node[midway,above,font=\tiny] {0/00};
%   \draw[dashed] (s0-t\t) -- (s2-t\tnext) node[midway,below,font=\tiny] {1/11};
% }
% 
% % From state 01
% \foreach \t in {0,1,2,3} {
%   \pgfmathtruncatemacro{\tnext}{\t+1}
%   \draw[solid] (s1-t\t) -- (s0-t\tnext) node[midway,above,font=\tiny] {0/11};
%   \draw[dashed] (s1-t\t) -- (s2-t\tnext) node[midway,below,font=\tiny] {1/00};
% }
% 
% % From state 10
% \foreach \t in {0,1,2,3} {
%   \pgfmathtruncatemacro{\tnext}{\t+1}
%   \draw[solid] (s2-t\t) -- (s1-t\tnext) node[midway,above,font=\tiny] {0/10};
%   \draw[dashed] (s2-t\t) -- (s3-t\tnext) node[midway,below,font=\tiny] {1/01};
% }
% 
% % From state 11
% \foreach \t in {0,1,2,3} {
%   \pgfmathtruncatemacro{\tnext}{\t+1}
%   \draw[solid] (s3-t\t) -- (s1-t\tnext) node[midway,above,font=\tiny] {0/01};
%   \draw[dashed] (s3-t\t) -- (s3-t\tnext) node[midway,below,font=\tiny] {1/10};
% }
% 
% % Legend
