...
\begin{tikzpicture}[scale=1.5]
% Axes
\draw[->] (0,0) -- (6,0) node[right] {\sffamily\small Power Density (mW/cm$^2$)};
\draw[->] (0,0) -- (0,4) node[above] {\sffamily\small Exposure Category};

% Bars for different exposure categories
% Safe zone (0-2)
\fill[green!40] (0,0.5) rectangle (2,1.2);
\node[right,font=\small] at (2.1,0.85) {ICNIRP Public Limit: 2 mW/cm$^2$};

% Occupational (0-10)
\fill[yellow!40] (0,1.7) rectangle (10/1.67,2.4);
\node[right,font=\small] at (6.1,2.05) {ICNIRP Occupational: 10 mW/cm$^2$};

% Typical applications
\fill[blue!30] (0,2.9) rectangle (0.5/1.67,3.6);
\node[right,font=\small] at (0.5,3.25) {Security Scanner: 0.5 mW/cm$^2$};

\fill[blue!30] (0,3.6) rectangle (1/1.67,4.3);
\node[right,font=\small] at (0.8,3.95) {Medical Imaging: 1 mW/cm$^2$};
...
\foreach \x in {0,2,4,6,8,10}
  \draw ({\x/1.67},0) -- ({\x/1.67},-0.1) node[below,font=\scriptsize] {\x};

% Danger zone marker
\draw[red,very thick,dashed] (10/1.67,0) -- (10/1.67,4.5);
\node[red,font=\small\bfseries,align=center] at (5.2,4.2) {Exceeds\\Occupational\\Limit};

% Safe margin annotation
\draw[<->,thick,green!50!black] (2/1.67,0.3) -- (10/1.67,0.3);
\node[below,font=\scriptsize,green!50!black] at (3.6,0.3) {5× safety margin};
