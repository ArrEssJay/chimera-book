...
\textbf{Key difference:} QAM uses both phase and amplitude, creating a rectangular grid that achieves higher spectral efficiency than PSK's circular constellation.
...
\subsection{Constellation Efficiency: 16-PSK vs 16-QAM}

Visual comparison showing why QAM outperforms PSK for the same number of symbols:

\begin{center}
\begin{tikzpicture}[scale=1.2]
% 16-PSK
\begin{scope}[shift={(0,0)}]
\node[above,font=\sffamily\bfseries] at (0,2.8) {16-PSK};
\draw[->] (-2.2,0) -- (2.2,0) node[right,font=\sffamily\small] {$I$};
\draw[->] (0,-2.2) -- (0,2.2) node[above,font=\sffamily\small] {$Q$};
\draw[very thin,gray!30] (-2,-2) grid[step=0.5] (2,2);
% Circle showing constant amplitude
\draw[thick,gray!50] (0,0) circle (1.5);
% 16 points on circle
\foreach \angle in {0,22.5,...,337.5} {
  \fill[black] ({1.5*cos(\angle)},{1.5*sin(\angle)}) circle (1.8pt);
}
\node[below=8pt,font=\small] at (0,-2.2) {Circular constellation};
\node[below=20pt,font=\scriptsize,align=center] at (0,-2.2) {Min distance: $d_{\min} = 0.39$};
\end{scope}

% 16-QAM
\begin{scope}[shift={(6,0)}]
\node[above,font=\sffamily\bfseries] at (0,2.8) {16-QAM};
\draw[->] (-2.2,0) -- (2.2,0) node[right,font=\sffamily\small] {$I$};
\draw[->] (0,-2.2) -- (0,2.2) node[above,font=\sffamily\small] {$Q$};
\draw[very thin,gray!30] (-2,-2) grid[step=0.5] (2,2);
% 16-QAM grid
...
\node[below=8pt,font=\small] at (0,-2.2) {Rectangular grid};
\node[below=20pt,font=\scriptsize,align=center] at (0,-2.2) {Min distance: $d_{\min} = 0.63$};
\end{scope}
\end{tikzpicture}
\end{center}

\textbf{Why QAM wins:} The rectangular grid uses 2D signal space more efficiently than a circle. Symbol spacing is maximized for a given average power, increasing minimum distance ($d_{\min}$) by $\sim$60\% and reducing BER by approximately 4~dB for the same transmitted power.
